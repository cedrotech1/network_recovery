...
export default defineConfig(({ mode }) => {
  ...
  const port = parseInt(env.VITE_DEV_PORT || '5473', 10)
  // Only set when the browser reaches Vite via a reverse proxy (e.g. nginx :80).
  // Local `npm run dev` must leave this unset or HMR connects to the wrong port.
  const hmrClientPort = env.VITE_HMR_CLIENT_PORT
    ? parseInt(env.VITE_HMR_CLIENT_PORT, 10)
    : null

  // When served under a subpath (e.g. /uat/), proxy API to the backend
  const apiProxy =
    base && base !== '/'
      ? {
          [`${base.replace(/\/$/, '')}/api`]: {
