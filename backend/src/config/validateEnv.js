...
export function validateEnv() {
  ...
  const proHost = process.env.PRO_DATABASE_HOST || devHost;
  const uatHost = process.env.UAT_DATABASE_HOST || devHost;
  const devName = process.env.DEV_DATABASE_NAME;
  const proName = process.env.PRO_DATABASE_NAME || devName;
  const uatName = process.env.UAT_DATABASE_NAME || 'network_recovery_uat';
